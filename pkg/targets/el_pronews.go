...
func (h El_ProNews) ParseList(client *http.Client) ([]*gofeed.Item, error) {
	...
	collection := doc.Find("article.post-article")
	if collection.Size() == 0 {
		return list, nil
	}

	collection.Each(func(i int, s *goquery.Selection) {
		href, hok := s.Find(".relative-title .wrap-post-title-cat a.article-link").Attr("href")
		if !hok {
			return
		}
		datetime := s.Find(".relative-title .wrap-post-meta .post-meta span.post-date").Text()
		t, err := time.ParseInLocation("02.01.2006 | 15:04", strings.TrimSpace(datetime), loc)
		if err != nil {
			return
		}
		item := &gofeed.Item{
			Title:           strings.TrimSpace(s.Find(".relative-title .wrap-post-title-cat a.article-link h3").Text()),
