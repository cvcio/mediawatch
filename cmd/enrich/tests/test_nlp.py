...
async def test_extract_stopwords() -> None:
    ...
    stopwords = await extract_stopwords(
        test_text, list(test_spacy_model.Defaults.stop_words)
    )
...
def test_extractive_summarization() -> None:
    test_spacy_model = spacy.load("el_core_news_md")
    doc = test_spacy_model(test_text)
...
async def test_extract_claims() -> None:
    test_spacy_model = spacy.load("el_core_news_md")
    doc = test_spacy_model(test_text)
    claims = await extract_claims(doc, list(test_spacy_model.Defaults.stop_words), 0.4)
    assert len(claims) == 6
    logging.info(claims)
...
async def test_extract_topics() -> None:
    ...
    topics = await extract_topics(test_text, model.topic_classification_pipeline)
    logging.info(topics)
    assert len(topics) == 2


# @pytest.mark.asyncio
# async def test_extract_named_entities() -> None:
#     model = AIModel("models/el.json")
#     entities = await extract_named_entities(
#         test_text, model.ner_classification_pipeline
#     )
#     logging.info(entities)
